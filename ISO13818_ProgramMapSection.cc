...
#include <iostream>
#include <map>
#define IMPLEMENTING_ISO13818_PROGRAMMAPSECTION
#include "ISO13818_ProgramMapSection.h"
#include "B10_Descriptors.h"

FIELDWIDTH_PREAMBLE(ISO13818_ProgramMapSection)
...
FIELDWIDTH_POSTAMBLE(ISO13818_ProgramMapSection, /**/)
...
#define SHORTDUMP
#ifdef SHORTDUMP
void ISO13818_ProgramMapSection::dump(std::ostream *osp) const {
   ...
   try {
	 ...
	 *osp << "    Stream #" << std::dec << snum << "[0x" << std::hex << elmpid << "] " << stream_type(sttype) << ": ";
	 ...
	 while (idx < es_tail) {
	    ...
	    if (ed_tag == B10_Descriptor::tag_video_decode_control) {
	       B10_Desc_VideoDecodeControl desc;
	       desc.setBuffer(*this, idx, len);
	       *osp << desc.video_encode_format_string() << " ";
	    } else if (ed_tag == B10_Descriptor::tag_stream_identifier) {
	       B10_Desc_StreamIdentifier desc;
	       desc.setBuffer(*this, idx, len);
	       *osp << "component_tag (for EIT)=" << desc.component_tag_string() << " ";
	    } else if (ed_tag == B10_Descriptor::tag_data_component) {
	       B10_Desc_DataComponent desc;
	       desc.setBuffer(*this, idx, len);
...
void ISO13818_ProgramMapSection::dump(std::ostream *osp) const {
   ...
   try {
	 ...
	 int desclen = byteAt(idx + 1);
	 *osp << "    descriptor=" << B10_Descriptor::getName(desctag)
	      << ", len=" << (int)desclen + 2
	      << std::endl;
	 idx += 2 + desclen;
      }
	 ...
	 *osp << "    --stream_type=" << std::hex << std::showbase << sttype << ": " << stream_type(sttype)
	 ...
	 while (idx < es_tail) {
	    int ed_tag = byteAt(idx);
	    int len = 2 + byteAt(idx + 1);
	    *osp << "        element descriptor="  << B10_Descriptor::getName(ed_tag)
		 << ", len=" << len
		 << std::endl;
	    if (ed_tag == B10_Descriptor::tag_video_decode_control) {
	       B10_Desc_VideoDecodeControl desc;
...
void ISO13818_ProgramMapSection::for_all_streams(StreamCallback scp, void *dtp) const {
   if (table_id() != TableID_TSProgramMapSection) {
      return;
   }
   try {
	 ...
	 int data_component_id = -1;
	 while (idx < es_tail) {
	    int ed_tag = byteAt(idx);
	    int len = 2 + byteAt(idx + 1);
	    if (ed_tag == B10_Descriptor::tag_stream_identifier) {
	       B10_Desc_StreamIdentifier desc;
	       desc.setBuffer(*this, idx, len);
	       ...
	       B10_Desc_DataComponent desc;
	       desc.setBuffer(*this, idx, len);
	       data_component_id = desc.data_component_id();
	    }
	    idx += len;
	 }
	 (*scp)(elmpid, program_number(), sttype, component_tag, data_component_id, dtp);
      }
   } catch (ByteArrayOverflowException e) {
      logger->error("buffer underflow!!");
   }
...
uint8 ISO13818_ProgramMapSection::version_number() const {
   return bit_field8(pos_version_number);
};

int ISO13818_ProgramMapSection::section_number() const {
   return bit_field8(pos_section_number);
};

bool ISO13818_ProgramMapSection::current_next_indicator() const {
   return bit_field1(pos_current_next_indicator);
}

int ISO13818_ProgramMapSection::last_section_number() const {
   return bit_field8(pos_last_section_number);
};

uint16 ISO13818_ProgramMapSection::PCR_PID() const {
   return bit_field16(pos_PCR_PID);
};

uint16 ISO13818_ProgramMapSection::program_number() const {
   return bit_field16(pos_program_number);
};

int ISO13818_ProgramMapSection::program_info_length() const {
